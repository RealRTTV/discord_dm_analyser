...
fn top_call_lengths(dms: &DirectMessages) -> Result<()> {
    ...
    println!("8 hour calls: {}", lengths.iter().filter(|&&delta| delta >= TimeDelta::hours(8)).count());
...
fn longest_time_between_different_users(dms: &DirectMessages) -> Result<()> {
    ...
    for (idx, (diff, first_content, second_content, first_id, second_id, first_timestamp, second_timestamp, messages_between)) in differences.iter().rev().take(25).enumerate() {
        let difference = TimeQuantity::from(diff.num_milliseconds() as usize);
        println!("{n}: diff = {difference}, messages_between = {messages_between}, first_timestamp = {first_timestamp}, second_timestamp = {second_timestamp}, first_id = {first_id}, second_id = {second_id} | first_content = {first_content:?} | second_content = {second_content:?}", n = idx + 1);
    }
    ...
    for (idx, (diff, first_content, second_content, first_id, second_id, first_timestamp, second_timestamp, messages_between)) in differences.iter().rev().take(25).enumerate() {
        let difference = TimeQuantity::from(diff.num_milliseconds() as usize);
        println!("{n}: messages_between = {messages_between}, diff = {difference}, first_timestamp = {first_timestamp}, second_timestamp = {second_timestamp}, first_id = {first_id}, second_id = {second_id} | first_content = {first_content:?} | second_content = {second_content:?}", n = idx + 1);
    }
...
fn words_and_characters_written(dms: &DirectMessages) -> Result<()> {
    ...
    for text in dms.messages.iter().filter_map(Message::as_text_message) {
        ...
        //     println!("{author}'s {milestone_nth} word was '{word}' (https://discord.com/channels/@me/{author_id}/{msg_id}) @ {datetime}", author = text.author.name.as_str(), milestone_nth = nth(milestone), author_id = text.author.id, msg_id = text.id, datetime = text.timestamp);
        // }
        entry.0 += words;
...
fn capitalization_rates(dms: &DirectMessages) -> Result<()> {
    ...
    for year in first_year..=last_year {
        ...
        for (author_idx, (capitalized, uncapitalized)) in quantities.into_iter().enumerate() {
            ...
            println!("{author_name}: {capitalized} / {total} ({pct:.2}%)", pct = 100.0 * capitalized as f64 / total as f64);
        }
    }
...
fn edit_rates(dms: &DirectMessages) -> Result<()> {
    println!("\n# Edit Rates");
    ...
    for year in first_year..=last_year {
        ...
        for (author_idx, (edited, unedited)) in quantities.into_iter().enumerate() {
            ...
            let author_name = dms.channel.authors[author_idx];
            println!("{author_name}: {edited} / {total} ({pct:.2}%)", pct = 100.0 * edited as f64 / total as f64);
        }
    }
